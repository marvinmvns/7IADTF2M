...
#===============================================================================
# Saúdelog - Script de Desinstalação para macOS
...
cat << "EOF"
  ...
  |____/ \__,_|\__,_|_|  \___|_|\___|_|\___/ \__, |
                                              |___/
  ══════════════════════════════════════════════════
  Desinstalação - macOS - VRP Optimizer
  ══════════════════════════════════════════════════
EOF
echo -e "${NC}"
...
if command -v ollama &> /dev/null; then
    ...
    echo ""
    if [[ $REPLY =~ ^[Ss]$ ]]; then
        brew uninstall ollama 2>/dev/null || print_info "Ollama não foi instalado via Homebrew"
        rm -rf ~/.ollama 2>/dev/null
        print_success "Ollama removido"
    else
